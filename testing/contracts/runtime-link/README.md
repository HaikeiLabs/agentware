...
| `wire/child-events.json` | The v1 child stdout JSONL parser: accepted events keep only allowlisted, bounded fields; unknown events are ignored; unknown `v` or outcome is `contract_mismatch`; lines over 4 KiB are `line_too_long`. Identity `agent_id` / `agents` are optional (older runtimes omit them); malformed agent entries are dropped and counted (`dropped_agents`) without failing the line. |
